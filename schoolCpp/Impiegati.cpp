...
int main(){
	...
	cout << endl <<"---- Persona con stipendio minimo ----" << endl << endl;
	float min=40000;
	int m;
	for(int i=0; i<n; i++){
		if(imp[i].getStipendio()<min){
			m=i;
			min=imp[i].getStipendio();
		}
	}
	cout << "Nome e Cognome: " << imp[m].getNome() << " " << imp[m].getCognome() << endl;
	cout << "Mansione: " << imp[m].getMansione() << endl;
	cout << "Stipendio: " << imp[m].getStipendio() << " euro/anno"<< endl << endl;
